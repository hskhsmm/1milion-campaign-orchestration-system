...
### 12차 구조적 결함 발견 → 13차 수정

**12차** (130만 재고, 150만 요청, ASG 3대): DB COUNT = 1,158,938 → **141,062건 유실** 🔴
- 원인 1: Queue 크기 부족 (MAX_QUEUE_SIZE=1M) → 트래픽 초과 시 overflow
- 원인 2: 재고 차감(DECR)과 큐 적재(LPUSH)가 비원자적 → Queue full 시 DECR은 됐지만 LPUSH 실패 → 202 반환했지만 DB에 없음, 복구 불가
- MCP 서버 실시간 GC 이상/Kafka lag 탐지 검증 ✅

**수정 (두 가지 동시 적용)**: check-decr-enqueue.lua 단일 Lua 원자화 + MAX_QUEUE_SIZE 1.5M
- 큐 만원 시 DECR 자체 미실행 → partial failure 원천 차단 (queue full → 429, 재고 차감 없음)
...
- 단, 13차에서 Redis Queue 최대 1.2M으로 1.5M 상한 미도달 → 원자화의 queue full 경로가 실제 발동되지 않음
- **원자화 경로(queue full → 429 + 재고 차감 없음)의 실제 검증은 T03에서 수행**
  - T03: MAX_QUEUE_SIZE=500K 임시 축소 → Queue 의도적으로 꽉 채움 → 429 반환 + Redis 잔여재고 + DB = total_stock 정합성 완벽 확인
...
| T03 | Queue 만원 429 | ✅ 통과 | MAX_QUEUE_SIZE=500K 임시 축소, Redis 남은 재고+DB=total_stock 정합성 완벽 |
...
### 스토리 아크 (이력서/자소서/면접 모두 이 흐름)

```
1. 동기: 선착순 캠페인 — "먼저 온 사람이 먼저 당첨" + 재고 초과 판매 0건
2. 병목 발견: HikariCP pending 980 → DB Row Lock이 근본 원인
3. 구조적 전환: DB 동기 → Redis-first v3 (API 응답 경로에서 DB 완전 제거)
4. 반복 실험: 14차 부하 테스트, 매 차수마다 병목 측정 → 원인 → 수정 사이클
5. 데이터 유실 발견: 12차 141K 유실 → 두 원인 분리 (Queue 크기 + 비원자성)
6. 원자화 해결 + T03 검증: Lua 단일 스크립트, queue full → 429 + 재고 차감 없음
7. 최종: TPS 246→3,737 (15배), 150만 정합성 완벽, 5xx 0건, 장애 T01~T07 전체 통과
```
...
| 항목 | 수치 | 출처 |
|------|------|------|
| 평균 TPS (14차) | ~3,737/s | Grafana 대시보드 |
| 피크 TPS | ~4,800/s | Grafana 대시보드 |
| 정합성 | 1,500,000건 (diff=0) | DB 쿼리 직접 확인 |
| 5xx 에러 | 0건 | Grafana 에러율 No data |
| 초기 대비 TPS 향상 | 246→3,737 (약 15배) | 1차~14차 누적 |
| Consumer 지연 개선 | 200ms→7.5ms | batchUpdate 전환 후 |
| Kafka 파티션 키 개선 | Consumer 지연 1.25s→200ms | 6차 테스트 (userId 키) |
| ASG 수평 확장 효과 | 1,220→2,014 TPS (+65%) | 8차→9차 비교 |

### 포폴 증거 사진 맵

| 파일 경로 | 내용 | 용도 |
|---------|------|------|
| `150만 테스트/스크린샷 2026-05-04 182904.png` | TPS 4.8K 피크, 5xx No data | **핵심 성능 증거** |
| `150만 테스트/KakaoTalk_20260504_183054474.png` | ASG 3대 CPU 97.3% | 서버 한계 측정 증거 |
| `150만 테스트/KakaoTalk_20260504_190438660.png` | DB diff=0, 1,500,000 완벽 | **정합성 증거** |
| `asg 적용 이후 50만 테스트/` | ASG 2대 적용 후 성능 | ASG 효과 증거 |
| `장애 테스트/T05/` | Kafka 브로커 1대 다운, TPS 3K~4.5K 유지, 5xx No data | 내결함성 증거 |
| `장애 테스트/T06/` | RDS SG 삭제/복구, 정합성 diff=0 | 장애 복구 증거 |
| `장애 테스트/T07/KakaoTalk_20260505_190141438.png` | DB 800K diff=0 (ASG 인스턴스 terminate 후) | **ASG 자가 복구 증거** |
| `장애 테스트/T03/` | Queue 500K 초과 → 429, 정합성 완벽 | Lua 원자화 검증 증거 |
| `100만/KakaoTalk_20260428_231435894_01.png` | RDS WriteIOPS 스파이크 (배치 INSERT) | DB 최적화 전/후 |

### Lua 스크립트 — 포폴 핵심 코드

`check-decr-enqueue.lua`: 5개 Redis 연산을 단일 원자 실행
```
1. active 플래그 체크 (-999: 비활성)
2. participated 키 체크 (-997: 중복 참여)
3. Queue 크기 체크 (-998: 만원 → DECR 미실행)
4. DECR 실행 (재고 차감)
5. LPUSH + participated SET (큐 적재 + 참여 이력)
```
→ Queue full 시 DECR 자체 미실행 = partial failure 원천 차단

### 기술 차별점 (신입 기준 드문 경험)

1. **14차 반복 실험** — 측정 → 원인 추적 → 수정 → 재실험 사이클
2. **실제 데이터 유실 발견** — 12차 141K 유실을 DB 쿼리로 직접 발견, 근본 원인 분리 분석
3. **ElastiCache CME 슬롯 제약 해결** — Lua 멀티키 접근 시 해시태그 설계 실제 경험
4. **완전한 인프라 IaC** — VPC/SG/ASG/CodeDeploy/ElastiCache/RDS 전체 Terraform
5. **OIDC + SSM 보안** — AWS Access Key 0개, SSH 포트 차단, SecureString 관리
6. **AI 운영 레이어** — Prometheus/CloudWatch 폴링 + P1/P2/P3 임계값 + Slack 알림 + MCP 도구 7개
7. **장애 테스트 T01~T07** — 실제 AWS 환경 SG 삭제, 인스턴스 terminate 등 실전 장애 주입

---

## 면접 핵심 멘트

**프로젝트 소개**: "선착순 캠페인 참여 시스템을 14차 반복 실험을 통해 고도화한 프로젝트입니다. 병목을 측정하고 원인을 추적해 수정한 뒤 재실험으로 검증하는 사이클을 반복했고, 최종적으로 평균 TPS ~3,737/s, 피크 ~4,800/s, 150만 정합성 완벽(diff=0), 5xx 0건을 달성했습니다. 단순 구현이 아니라 데이터로 문제를 발견하고 해결한 실험 중심 프로젝트입니다."
...
**구조적 결함 발견 및 해결 (12차→13차)**: "12차에서 두 가지 문제를 동시에 발견했습니다. Queue 크기 부족(1M)과 DECR/LPUSH 비원자성. Queue full 시 DECR은 됐지만 LPUSH 실패 → 사용자는 202 받았지만 DB에 없음 → PENDING/DLQ 기록도 없어 복구 불가. 두 수정을 함께 적용(Lua 원자화 + MAX_QUEUE_SIZE 1.5M)해 13차 DB COUNT 1,500,000 정합성 완벽 달성. 단, 13차에서 Queue가 1.2M까지만 차서 원자화 경로가 실제 발동되지 않았고, 원자화(queue full → 429 + 재고 차감 없음) 자체는 T03에서 별도 검증했습니다. T03에서 MAX_QUEUE_SIZE를 500K로 낮춰 의도적으로 꽉 채운 뒤 Redis 잔여재고 + DB = total_stock 정합성을 확인했습니다."
...
**장애 테스트 (T01~T07 전체 통과)**: "7개 시나리오 전체 실제 AWS 환경에서 검증 완료. T03에서 MAX_QUEUE_SIZE=500K로 임시 축소해 Queue를 의도적으로 꽉 채운 뒤 429 반환 + 재고 차감 없음 + Redis 잔여재고 + DB = total_stock 정합성 확인 — 이게 Lua 원자화 경로의 실제 검증. T05에서 VUS=500으로도 Kafka lag 스파이크 → 즉시 해소 확인. T07 인스턴스 terminate 후 TPS 80% 급락 → 3~4분 완전 복구, 5xx 0건, DB diff=0"
