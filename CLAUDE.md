...
- [x] `application-local.yml` — `cluster.nodes: localhost:7001,7002,7003` 추가
...
#### 📌 terraform apply 전 수동 작업
- `terraform destroy -target=aws_elasticache_replication_group.redis` (기존 단일 노드 삭제)
- `terraform apply` (CME 3샤드 생성, ~10분)
- 콘솔에서 Configuration Endpoint 확인 후 SSM 파라미터 등록:
  - `/batch-kafka/prod/SPRING_DATA_REDIS_CLUSTER_NODES` — `<config-endpoint>:6379`
  - `/batch-kafka/prod/REDIS_EXPORTER_ADDR` — `redis://<config-endpoint>:6379`
  - `/batch-kafka/prod/SPRING_DATA_REDIS_HOST` 삭제
...
| 6 | Redis Cluster 3샤드 + AOF | ✅ 코드 완료 (2026-04-25) — terraform apply 대기 |
