...
현재 1단계에서는 실행 파일을 만들지 않고, 기존 수동 절차를 기준선으로 문서화한다. 이후 단계에서 이 문서의 순서를 그대로 Ansible playbook으로 옮긴다.
...
| 단계 | 파일 | 목적 |
| --- | --- | --- |
| 2 | `ops/ansible.cfg`, `ops/inventory/aws_ec2.yml`, `ops/group_vars/all.yml` | Ansible 실행 기본값과 AWS dynamic inventory 준비 |
